    ...
    <div className="liste-utilisateurs">
      {utilisateurs.map((utilisateur) => (
        <article className="carte-utilisateur" key={utilisateur.id}>
          ...
          <div className="badges-utilisateur"><span className={`badge-role role-${utilisateur.role.toLocaleLowerCase("fr")}`}>{libellesRoles[utilisateur.role]}</span><span className={utilisateur.actif ? "badge-statut statut-actif" : "badge-statut statut-inactif"}>{utilisateur.actif ? "Actif" : "Inactif"}</span>{utilisateur.peutGererPedagogie && <span className="badge-pedagogie">Droit pédagogique</span>}</div>
  ...
  return (
    <aside className="panneau-utilisateur" aria-labelledby="titre-formulaire-utilisateur">
      ...
      <form onSubmit={soumettre} className="formulaire-utilisateur">
        ...
        {saisie.role !== "RECRUTEUR" && <label className="ligne-case"><input type="checkbox" checked={saisie.peutGererPedagogie} onChange={(event) => modifier("peutGererPedagogie", event.target.checked)} /> Autoriser la gestion pédagogique</label>}
