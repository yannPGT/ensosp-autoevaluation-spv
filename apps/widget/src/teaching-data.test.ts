...
function donnees(){return construireDonneesPedagogiques(
  ...
  {id:[2],Uid:["v1"],Fiche:[1],NumeroVersion:[1],FichierPDF:[["L",101]],NomFichier:["guide.pdf"],TailleOctets:[1024],EmpreinteSHA256:["abc"],Auteur:[7],EstPubliee:[true],DatePublication:[1_700_000_000],DateFinValidite:[null]},
...
describe("données du module pédagogique",()=>{
  ...
  it("résout le périmètre, la version numérique active et les liaisons",()=>{const d=donnees();expect(d.fiches[0]).toMatchObject({perimetre:"Groupement Nord",versionActive:"1.0",versions:1,liaisons:1});expect(d.versions[0]).toMatchObject({numero:"1.0",attachmentId:101,auteur:"Yann Admin"});expect(d.liaisons[0]).toMatchObject({codeIndicateur:"IND_01",rouge:true});});
