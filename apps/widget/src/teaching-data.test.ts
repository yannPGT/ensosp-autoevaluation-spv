...
describe("données du module pédagogique",()=>{
  ...
  it("protège les codes et les liaisons fonctionnelles",()=>{const d=donnees();expect(()=>validerFiche({code:"FICHE_IND_01",titre:"Doublon",description:"",perimetreId:0,actif:true},d)).toThrow(/existe déjà/);expect(()=>validerLiaison({ficheId:1,indicateurId:12,rouge:false,orange:false,actif:true},d)).toThrow(/rouge ou orange/);expect(()=>validerLiaison({ficheId:1,indicateurId:11,rouge:true,orange:true,actif:true},d)).toThrow(/déjà lié/);});
