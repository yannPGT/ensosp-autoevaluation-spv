...
export async function chargerSessionEvaluation(utilisateur: UtilisateurCourant): Promise<SessionEvaluation> {
  ...
  (indicateurs.id ?? []).forEach((v, i) => {
    const id = nombre(v);
    if (id) codes.set(id, texte(indicateurs.Code?.[i]));
  });

  const resultat: Record<string, Niveau> = {};
  (reponses.id ?? []).forEach((_, i) => {
    if (referenceId(reponses.Evaluation?.[i]) !== evaluationId) return;
    const code = codes.get(referenceId(reponses.Indicateur?.[i]) ?? 0);
...
export async function creerEvaluation(utilisateur: UtilisateurCourant): Promise<number> {
  ...
  const ui = (utilisateurs.id ?? []).findIndex((v) => nombre(v) === utilisateur.id);
  const perimetre = referenceId(utilisateurs.PerimetrePrincipal?.[ui]);
  if (!perimetre) throw new Error("Votre périmètre principal n’est pas configuré.");

  const campagneIndex = (campagnes.id ?? []).findIndex((_, i) =>
    texte(campagnes.Statut?.[i]) === "OUVERTE"
    && (!referenceId(campagnes.Perimetre?.[i]) || referenceId(campagnes.Perimetre?.[i]) === perimetre)
  );
  ...
  const i = (evaluations.Uid ?? []).findIndex((v) => v === uid);
  const id = nombre(evaluations.id?.[i]);
  if (!id) throw new Error("Le brouillon d’évaluation n’a pas pu être retrouvé.");
  return id;
}
...
export async function enregistrerReponse(evaluationId: number, code: string, niveau: Niveau): Promise<void> {
  ...
  if (texte(evaluations.Statut?.[ei]) !== "BROUILLON") throw new Error("Cette évaluation n’est plus modifiable.");
  ...
  const ii = (indicateurs.Code ?? []).findIndex((v) => v === code);
  const indicateurId = nombre(indicateurs.id?.[ii]);
  if (!indicateurId) throw new Error(`L’indicateur ${code} est absent du référentiel Grist.`);

  const ri = (reponses.id ?? []).findIndex((_, i) =>
    referenceId(reponses.Evaluation?.[i]) === evaluationId
...
export async function validerEvaluation(evaluationId: number, utilisateur: UtilisateurCourant): Promise<ResultatValidation> {
  ...
  try {
    ...
    if (!feuilleId) {
      const uid = crypto.randomUUID();
      await api.applyUserActions([["AddRecord", "FeuillesRoute", null, {
        Uid: uid,
        Evaluation: evaluationId,
        Recruteur: recruteur,
        Perimetre: perimetre,
      ...
      const table = await api.fetchTable("FeuillesRoute");
      const fi = (table.Uid ?? []).findIndex((v) => v === uid);
    ...
    const actions = lignes
      ...
      .map((r) => ["AddRecord", "ActionsProgres", null, {
        Uid: crypto.randomUUID(),
        FeuilleRoute: feuilleId,
        Evaluation: evaluationId,
        Reponse: r.id,
        Recruteur: recruteur,
        Perimetre: perimetre,
        Indicateur: r.indicateur,
        FicheVersion: versionParIndicateur.get(r.indicateur) ?? null,
  ...
  try {
    await api.applyUserActions([["AddRecord", "JournalAudit", null, {
      ...
      ObjetUid: texte(evaluations.Uid?.[ei]) || String(evaluationId),
      Action: "VALIDATION",
...
function trouverFeuille(t: TableGrist, e: number): number | null {
  const i = (t.id ?? []).findIndex((_, x) => referenceId(t.Evaluation?.[x]) === e);
  return nombre(t.id?.[i]);
}

function exigerApi() {
  const a = obtenirDocApiGrist();
  if (!a) throw new Error("Cette opération est disponible uniquement depuis le widget Grist.");
  return a;
}
...
function referenceId(v: unknown): number | null {
  ...
  if (Array.isArray(v)) {
    for (let i = v.length - 1; i >= 0; i -= 1) {
      if (typeof v[i] === "number" && (v[i] as number) > 0) return v[i] as number;
    }
  }
  return null;
}
