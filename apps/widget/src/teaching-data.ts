...
export type NiveauPedagogique="ROUGE"|"ORANGE";
export interface SaisieFiche { id?:number; code:string; titre:string; description:string; perimetreId:number; actif:boolean; indicateurId:number; niveau:NiveauPedagogique; }
...
export async function enregistrerFiche(x:SaisieFiche,d:DonneesPedagogiques,utilisateurId:number):Promise<void>{
  ...
  if(ficheId)await api.applyUserActions([["UpdateRecord","FichesEnseignement",ficheId,champs],["AddRecord","JournalAudit",null,{Uid:crypto.randomUUID(),Acteur:utilisateurId,Perimetre:x.perimetreId||null,TypeObjet:"FICHE",ObjetUid:String(ficheId),Action:"MODIFICATION_FICHE",Resume:`Fiche ${x.code} modifiée`}]]);
    ...
    if(!ficheId)throw new Error("La fiche créée n’a pas pu être retrouvée pour enregistrer son indicateur.");
  }
  const liaisons=d.liaisons.filter(l=>l.ficheId===ficheId),liaison=liaisons.find(l=>l.indicateurId===x.indicateurId)??liaisons[0];
