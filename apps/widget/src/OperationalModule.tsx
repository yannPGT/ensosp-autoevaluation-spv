...
function FormDecision({ action: a, utilisateur, notifier, recharger }: { action: ActionMetier; utilisateur: UtilisateurCourant; notifier: (t: "succes" | "erreur", x: string) => void; recharger: () => void }) {
  ...
  return <form className="carte-action-progres" onSubmit={submit}><EnteteAction action={a} /><p><strong>Fiche d’enseignement :</strong> {a.ficheVersionId ? a.fiche : "Aucune fiche disponible lors de la déclaration"}</p><p><strong>Déclaration :</strong> {a.ficheVersionId ? (a.priseEnCompteFiche ? "Prise en compte de la fiche d’enseignement réalisée" : "Prise en compte non confirmée") : "Progression déclarée sans ressource pédagogique disponible"}</p>{a.commentaireRecruteur && <p><strong>Commentaire du recruteur :</strong> {a.commentaireRecruteur}</p>}<div className="grille-decision"><label>Décision<select value={decision} onChange={(e) => setDecision(e.target.value as DecisionValidation)}><option value="VALIDEE">Valider le passage au vert</option><option value="COMPLEMENT_DEMANDE">Demander un complément</option><option value="REFUSEE">Refuser</option></select></label>{decision === "VALIDEE" && <label>Nouveau niveau<select value="VERT" disabled><option value="VERT">VERT</option></select></label>}</div><label>Commentaire<textarea rows={3} value={commentaire} onChange={(e) => setCommentaire(e.target.value)} required={decision !== "VALIDEE"} /></label><button disabled={encours || Boolean(a.ficheVersionId && !a.priseEnCompteFiche)}>{encours ? "Enregistrement…" : "Enregistrer la décision"}</button></form>;
}
